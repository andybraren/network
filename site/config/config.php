...
c::set('routes', array(
  ...
  // SAVE PAGES - OLLLLLLDDDDD
  // Only seems to work on main root pages at the moment
  // Occasionally would break, updating only the modified fields and text, and deleting the title and everything else.
	array(
		'pattern' => array('saveold', '(.+saveold)'), // matches any url ending in save
		'method' => 'POST',
		'action'  => function() {
      try {
        ...
        if(isset($_POST["__page__"])) {
          // removes the base url and extra / from __page__
          //$targetpage = str_replace(parse_url(site()->url(), PHP_URL_PATH) . "/", "", $_POST["__page__"]);
          $targetpage = str_replace(parse_url(site()->url(), PHP_URL_PATH) . "/", "/", $_POST["__page__"]);
          // returns projects/oreo-decreamer for example
        ...
        if(isset($_POST["title"])) {
          ...
          $currentpath = kirby()->roots()->content() . '/' . $targetpage; // /var/www/drewbaren.com/maker/content/projects
          $foldertitle = seoURL(trim(strip_tags($_POST["title"])));
          //$redirecturl = site()->url() . '/' . $foldertitle;
          $redirecturl = site()->url() . '/projects/' . $foldertitle;
          //$newpath = kirby()->roots()->content() . '/' . str_replace(basename(page()->diruri()),'',page()->diruri()) . $foldertitle;
          $newpath = kirby()->roots()->content() . '/projects/' . str_replace(basename(page()->diruri()),'',page()->diruri()) . $foldertitle;
